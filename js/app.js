...
(function () {
  ...
  function submitWork() {
    ...
    // 只写一句"正在提交…"和卡死没区别 —— 家长在那头先刷新一次，
    // 看到的就是"才到 26 条"（其实还在传）。所以每批落地报一次进度。
    F.flushWork(function (p) {
      app.submitMsg = '正在提交… 第 ' + p.batch + ' / ' + p.batches + ' 批' +
        '（已传 ' + p.sent + ' / ' + p.total + ' 条）';
      render();
    }).then(function (r) {
      if (r && r.ok) {
        ...
        // 分批传的时候，失败往往发生在半路：前面几批已经到家长那头了。
        // 只说"没提交上去"会让孩子以为全白写了 —— 事实上再点一次只补剩下的。
        var sent = (r && r.sent) || 0;
        var err = (r && r.error) || '网络不通';
        app.submitMsg = sent
